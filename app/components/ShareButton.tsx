...
export default function ShareButton({ noteId, initialIsPublic, initialToken }: Props) {
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-8 z-50 bg-white border border-gray-200 rounded-xl shadow-xl p-4 w-72">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold text-gray-800">Bagikan Catatan</span>
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg mb-3">
              ...
              <button
                onClick={toggle}
                disabled={toggling}
                className={`relative w-10 h-6 rounded-full transition-colors disabled:opacity-50 ${isPublic ? "bg-green-500" : "bg-gray-300"}`}
              >
                {toggling ? (
            ...
            {isPublic && shareUrl && (
              <div className="flex gap-2">
                <input
                  readOnly
                  value={shareUrl}
                  className="flex-1 text-xs border border-gray-300 rounded-lg px-2 py-1.5 bg-gray-50 text-gray-600 truncate"
                />
                <button
                  onClick={copy}
